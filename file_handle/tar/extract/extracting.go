...
func Extracting(srcFile io.Reader, dst string, isGzip bool) (err error) {
	// 实例化一个 tar 读取器
	...
	for {
		...
		// 这个变量保存的有可能是目录，有可能是文件，所以就叫 dstPath
		...
		case tar.TypeDir: // 如果是目录时候，创建目录
			// 判断下目录是否存在，不存在就创建
			if b := ExistDir(dstPath); !b {
				// 使用 MkdirAll 不使用 Mkdir ，就类似 Linux 终端下的 mkdir -p，
				// 可以递归创建每一级目录
				if err := os.MkdirAll(dstPath, 0775); err != nil {
					return err
				}
			}
