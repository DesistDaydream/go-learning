...
func main() {
	// New 创建一个名为 Go template demo 的未定义模板。此时模板中无任何内容，模板内容以及要传递的值都没有，仅仅是生成一个模板指针，用来后续调用。
	// 这种行为类似于 open 一个文件，或者 new 一个数组之类，或者 open 一个数据库，或者 open 一个 redis 等等。
	// ！！！Notes: 若是要使用 ParseFilse() 解析一个文件，则 New 的参数必须与文件名相同！！！
	t := template.New("Go template demo")

	// Parse() 将参数传进来的文本解析为模板的主体。
	// 如果用一个文件来类比的话，现在就是有一个名为"Go template demo"的文件，这个文件的内容为"这是一个 {{ . }} 模板"
	t, _ = t.Parse("这是一个最简单的 {{ . }} 模板示例\n")

	// 上述两个步骤可以合并为一行代码，示例如下
	// 这是一个没有 Must 验证的代码
	// t, _ := template.New("Go template demo").Parse("这是一个 {{ . }} 模板")
