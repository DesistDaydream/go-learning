...
//定义一个函数，并声明形参i和o，并指明类型为int，未定义返回值
func Parameter(i int, o int) {
    fmt.Println(i)
    fmt.Println(o)
}

//定义一个函数，计算3个参数相乘的结果，指定了3个形参和1个返回值
func MultiPly3Nums(a int, b int, c int) int {
    return a * b * c
}

//定义一个函数，省略形参(注意加上括号，否则第一个括号内就编程该函数的参数而不是返回值了)，直接声明返回值的类型
func ThreeValues() (int, int, float32) {
    return 5, 6, 7.5
}

//与后面两个函数组合使用，用于输出后面两个函数的返回值
//后面两个函数指的是getX2AndX3和getX2AndX3_2
//主要用于说明返回值是否指明具体的变量名
func PrintValues() {
    fmt.Printf("num = %d, 2x num = %d, 3x num = %d\n", num, numx2, numx3)
}
//定义函数，指定参数input的类型为int，指定两个无名的int类型的返回值
func getX2AndX3(input int) (int,int) {
    return 2 * input, 3 * input
}
//定义函数，指定参数input的类型为int，还有两个有名的int类型的返回值。使用命名返回值的话，就算只有一个也要加上小括号
